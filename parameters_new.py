...
def parameters(ss, path_file, K=0):
    ...
    for fname in tqdm(os.listdir(path_file)):
        if fname.lower().endswith(('.pcd', '.txt', '.las', '.laz', '.h5')):
            ...
            down_point = 2*(pc_tree.points.max(axis=0)
                            [2]-pc_tree.points.min(axis=0)[2])/3+pc_tree.points.min(axis=0)[2]
...
if __name__ == "__main__":
    ...
    path_file = os.path.join(
        ss.path_base, ss.step1_folder_name, ss.step2_folder_name, ss.step3_folder_name)
